...
export default function Surpreenda() {
  ...
  // Cards com valores e cores definidas para alternar
  const cards = [
    ...
    <div className="flex flex-col items-center min-h-screen bg-cover bg-center bg-no-repeat px-4 pt-6"
      ...
      <div className="text-center mb-8">
        <h1 className="text-3xl font-bold text-black leading-snug">
          TUDO SOBRE OS <br /> SEUS PLAYS
        </h1>
        <p className="text-green-500 text-lg mt-2">você em dados</p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 w-full max-w-md mb-8">
        {cards.slice(0, 2).map((card, index) => (
          <div
            key={index}
            className={`border-2 border-orange-400 rounded-2xl p-4 flex flex-col items-center justify-center ${colorMap[card.color]}`}
          >
            <span className="text-2xl font-bold">{card.value}</span>
            <span className="text-center mt-2">{card.title}</span>
          </div>
        ))}
        ...
        <div
          className={`border-2 border-orange-400 rounded-2xl p-4 flex flex-col items-center justify-center ${colorMap[cards[6].color]} col-span-2`}
        >
          <span className="text-2xl font-bold">{cards[6].value}</span>
          <span className="text-center mt-2">{cards[6].title}</span>
        </div>
      </div>
    </div>
  );
